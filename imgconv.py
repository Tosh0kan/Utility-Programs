...
async def img_convert(path_outer: str, fmt: str) -> Image:
    def file_convert(count, path_inner: str, fmt: str, batch: bool = False) -> Image:
        if not batch:
            ...
            print(f"Saved file {count+1}/{len(file_list)}")
    ...
    if Path(path_outer).is_file():
        ...
        try:
            ...
            pass
        start = dt.now()
        await asyncio.gather(*(asyncio.to_thread(file_convert, i, e, 'png', batch=True)
